          ...
          if (AnnotatedFunction == F) {
            ...
            if (GlobalVariable *GV = dyn_cast<GlobalVariable>(
              ...
              if (ConstantDataArray *Anno =
                ...
                // Lowercase to match readAnnotate()'s behavior and stay
                // robust to any future annotate("Mixed Case") usage --
                // every current call site matches lowercase literals
                // (e.g. "linearmba"), so this is defense-in-depth, not
                // a behavior change for existing annotations.
                return Anno->getAsCString().lower();
              }
            }
          }
        }
...
// --- Polaris utility functions for LinearMBA and AliasAccess ---

uint64_t llvm::getRandomNumber() {
  return (((uint64_t)rand()) << 32) | ((uint64_t)rand());
}

uint32_t llvm::getUniqueNumber(std::vector<uint32_t> &rand_list) {
...
void llvm::getRandomNoRepeat(unsigned upper_bound, unsigned size,
