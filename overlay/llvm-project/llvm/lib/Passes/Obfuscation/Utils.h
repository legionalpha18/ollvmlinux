...
namespace llvm{
    ...
    // Opaque-pointer-safe annotation reader (works on LLVM 15+/opaque
    // pointers). Prefer this over readAnnotate() above, which relies on a
    // BitCast ConstantExpr pattern that no longer exists once pointers are
    // opaque and will silently return "" on modern LLVM.
